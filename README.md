...
Профессиональный canvas-редактор на Vue 3 + TypeScript. Пять инструментов рисования, история действий (Undo/Redo) и современный UI на Tailwind.
...
├── widgets/          # составные UI-блоки (toolbar)
├── features/         # пользовательские сценарии
│   ├── select-tool/  # выбор активного инструмента
│   ├── undo-redo/    # отмена / повтор
│   ├── export-png/   # скачивание рисунка
│   └── brush-settings/ # цвет и толщина
├── entities/         # бизнес-сущности
│   ├── canvas/       # CanvasEngine, SnapshotCommand
│   ├── tool/         # BrushTool, LineTool, RectangleTool, CircleTool, EraserTool, ToolFactory
│   └── editor/       # useEditor — оркестрация canvas + features
└── shared/           # переиспользуемый код
    ├── lib/          # ITool, ToolContext, ICommand, CommandHistory
    ├── types/        # Point, ToolType, BrushSettings
    ├── config/       # размеры canvas, дефолты кисти
    └── ui/           # BaseButton, иконки
```
...
`CanvasEngine` и `CommandHistory` менять не нужно.
